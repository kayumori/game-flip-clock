...
- **登山** … 正しく打てた単語の数だけ山を登る
- **スコア＆コンボ** … 連続正解でコンボが伸びてボーナス倍率アップ、ミスでコンボはリセット
- **〇〇級判定** … 登った高さ（=正しく打った文字数）に応じて、実在の山「ふもと → 笠山 → 高尾山 → 筑波山 → 富士山 → エベレスト級」などを判定
- **3コース** … EASY（45秒）／ NORMAL（60秒）／ HARD（90秒・ことわざ/早口言葉）。選ぶと 3・2・1 のカウントダウンで開始
- 結果に **○m 〇〇級 ／ SCORE ／ 入力スペル数 ／ ミス回数** を表示

> ローマ字入力はキーボード準拠（「ん」=`nn`、「っ」=次の子音を重ねる）。遊ぶときは一度ウィジェットをクリックしてキーボード入力を有効にしてください（スマホはタップでキーボードが出ます）。
